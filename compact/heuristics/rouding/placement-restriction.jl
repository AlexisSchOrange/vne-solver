...
function solve_heuristic_relax(instance)
    ...
    elseif selection_nodes == 2
        ...
        for v_node in vertices(v_network)
            vals = [(x_values[v_node, s_node] + sum(x_values[v_node, neighbor]/3 for neighbor in neighbors(s_network, s_node))) for s_node in vertices(s_network)]
            sorted = sortperm(vals, rev=true)
            placement_restriction[v_node] = sorted[1:nb_s_nodes]
    ...
    elseif selection_nodes == 4 # some more variety between each vnode
        ...
        for v_node in vertices(v_network)
            ...
            for i in 1:nb_s_nodes
                vals = [(x_values[v_node, s_node] 
                        + sum(x_values[v_node, neighbor]/5 for neighbor in neighbors(s_network, s_node))
                        - sum(x_values[v_node, neighbor]/5 for neighbor in intersect(neighbors(s_network, s_node), selec); init=0.0)
                        - sum(x_values[already_chosen_vnode, s_node]/5 for already_chosen_vnode in 1:(v_node-1); init=0.0)) 
                            for s_node in vertices(s_network)]
                sorted = sortperm(vals, rev=true)
                i_truc = 1
                stop = false 
