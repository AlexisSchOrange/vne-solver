...
function solve_subgraph_decompo(instance; time_max = 100, v_node_partitionning = [], nb_part = -1)
    ...
    nb_parts_init = floor(Int, nv(s_network) / (nb_max_node_subgraphs))
    nb_nodes_per_snsubgraph_init = max(15, 1.4 * nb_max_node_subgraphs)
    ...
    while keep_on
        ...
        for couple in sorted_subpb[1:nb_pricer_to_do]
            ...
            overall_obj += (reduced_cost/nb_pricer_to_do)
        end

        @printf("Iter %2d  CG bound: %10.3f  lower bound: %10.3f  %5d column  time: %5.2fs  average reduced cost: %10.3f \n",
                    nb_iter, cg_value, lower_bound, nb_columns, time_overall, overall_obj)
