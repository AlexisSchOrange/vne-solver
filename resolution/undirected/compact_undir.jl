...
function generate_dir_sn(instance)
    ...
    s_network_dir = MetaGraph(
        ...
        Dict,
        Dict,
        Dict(:name => "Whatever...", :type => "Substrate", :directed => true)
    )    
    for s_node in vertices(instance.s_network)
        add_vertex!(s_network_dir, s_node, instance.s_network[s_node])
    end
    for s_edge in edges(instance.s_network)
        add_edge!(s_network_dir, src(s_edge), dst(s_edge), copy(instance.s_network[src(s_edge), dst(s_edge)]))
        s_network_dir[src(s_edge), dst(s_edge)][:cost] = instance.s_network[src(s_edge), dst(s_edge)][:cost]
        add_edge!(s_network_dir, dst(s_edge), src(s_edge), copy(instance.s_network[src(s_edge), dst(s_edge)]))
        s_network_dir[dst(s_edge), src(s_edge)][:cost] = instance.s_network[src(s_edge), dst(s_edge)][:cost]
    end
...
function solve_compact_1vn(instance, time_solver = 30)
