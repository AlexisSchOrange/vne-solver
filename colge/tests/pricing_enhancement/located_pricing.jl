...
function solve_subgraph_decompo_1vn(instance, v_node_partitionning)
    ...
    while keep_on && nb_iter < nb_iter_max
        ...
        #=
        println("\nDual prices cap nodes:")
        for s_node in vertices(s_network)
            if (dual_costs.capacity_s_node[s_node] < -0.1)
                println("   For $s_node: $(dual_costs.capacity_s_node[s_node])")
            end
        end

        println("\nDual prices cap edges:")
        for s_edge in edges(s_network)
            if (dual_costs.capacity_s_edge[s_edge] < -0.1)
                println("   For $s_edge: $(dual_costs.capacity_s_edge[s_edge])")
            end
        end
        

        println("\nDual prices departure:")
        for v_edge_co in vn_decompo.v_edges_master
            println("   For $v_edge_co:")   
            for s_node in vertices(s_network)
                if ((dual_costs.departure[v_edge_co][s_node])< -0.1) 
                    println("       For $s_node: $(dual_costs.departure[v_edge_co][s_node])")
                end
            end
        end
        

        println("\nDual prices conservation flow ")
        for v_edge_co in vn_decompo.v_edges_master
            println("   For $v_edge_co:")
            for s_node in vertices(s_network)
                if (dual_costs.flow_conservation[v_edge_co][s_node])< -0.1 || (dual_costs.flow_conservation[v_edge_co][s_node]) >0.1
                    println("       For $s_node: $(dual_costs.flow_conservation[v_edge_co][s_node])")
                end
            end
        end
        =#
