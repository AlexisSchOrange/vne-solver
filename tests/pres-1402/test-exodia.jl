...
function solve_all_instances(path)
    ...
    overall_df = DataFrame(Gr=String[], Gs=String[], Algo=String[], Res1s=Int[], Res10s=Int[], Res60s=Int[], Res300s=Int[])
    ...
    for vn in vns
        ...
        for sn in sns
            ...
            val_ilp_300s = ceil(Int, solve_compact(instance, 300, false))
            #val_ilp_600s = ceil(Int, solve_compact(instance, 600, true))
            push!(overall_df, (instance.v_network[][:name], instance.s_network[][:name], "ILP", val_ilp_1s, val_ilp_10s, val_ilp_60s, val_ilp_300s)) 
            
            
            #=
            nb_part = 5
            if vn[][:name] == "W5_AW9"
                print("Well I need 5 part...")
                nb_part = 5
            end
            val_cg_1s = ceil(solve_subgraph_decompo(instance, 1, [], nb_part))
            val_cg_10s = ceil(solve_subgraph_decompo(instance, 10, [], nb_part))
            val_cg_60s = ceil(solve_subgraph_decompo(instance, 60, [], nb_part))
            val_cg_300s = ceil(solve_subgraph_decompo(instance, 300, [], nb_part))
            push!(overall_df, (instance.v_network[][:name], instance.s_network[][:name], "Colge", val_cg_1s, val_cg_10s, val_cg_60s, val_cg_300s)) 
