...
function solve_all_instances_colge(path)
    ...
    overall_df = DataFrame(Gr=String[], Gs=String[], Algo=String[], Times=Int[], NbSubG=Int32[], Res=Int32[], CGval=Float32[], LGBound=Float32[], nbcol=Int32[])
    ...
    for vn in vns
        ...
        for sn in sns
            ...
            for time_limit in [3600]
                ...
                CSV.write("xp_colge_$(last(path, 6)[1:5]).csv", overall_df)
            end
        end
    end
